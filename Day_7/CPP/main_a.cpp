...
int getSumOfSizesOfBelow100KDirs(std::shared_ptr<Directory> root)
{
	int sizeSum = 0;
	for (auto e : root->getElements())
	{
		...
		if (subdir)
		{
			...
			sizeSum += getSumOfSizesOfBelow100KDirs(subdir);
		}
	}
	return sizeSum;
}
...
int main()
{
	...
	int below100KDirsSum = getSumOfSizesOfBelow100KDirs(rootDir);
	std::cout << "The total sum of the sizes of directories bellow 100K is: " << below100KDirsSum << "\n";
